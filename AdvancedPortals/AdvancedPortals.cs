...
namespace AdvancedPortals
{
    ...
    public class AdvancedPortals : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            minTeleportItemDur = SyncedConfig("General", "minTeleportItemDur", 0.5f, "Minimum durability to allow item to be teleported through without using a thunderstone.");
            ...
            _ancientPortalMinItemDur = SyncedConfig("Portal 1 - Ancient", "Ancient Portal Minimum Durrability", 0.5f, "Minimum durability to allow item to be teleported through the Ancient Portal using a thunderstone.");
            ...
            _obsidianPortalMinItemDur = SyncedConfig("Portal 2 - Obsidian", "Obsidian Portal Minimum Durrability", 0.5f, "Minimum durability to allow item to be teleported through the Obsidian Portal using a thunderstone.");
            ...
            _blackMarblePortalMinItemDur = SyncedConfig("Portal 3 - Black Marble", "Black Marble Portal Minimum Durrability", 0.5f, "Minimum durability to allow item to be teleported through the Black Marble Portal using a thunderstone.");
            ...
            if (_ancientPortalEnabled.Value)
            {
                LoadBuildPiece(assetBundle, "portal_ancient", new PieceDef()
                {
                    ...
                    OnPrefabAdded = (prefab =>
                    {
                        ...
                        foreach (var itemName in advPortal.AllowedItems)
                        {
                        ...
                        itemDrop.m_description += $"\n{Localization.instance.Localize("$txt_can_teleport")}: {(advPortal.AllowEverything ? Localization.instance.Localize("$txt_anything") : string.Join(", ", allowedNames))}";
                    })
                });
            }
